...
#define INIT_SEQ_NUM 0x12345678

#define ROUTER_WIFI_MAC 0x114514114514
#define ROUTER_WIFI_IP 0xC0A81F01
#define PORT 1145
...
private:
	enum TransportState {
		...
		DNSSending, DNSReceiving,
		TCPSynSending, TCPSynReceiving,
		TCPAckSending, TCPGetSending,
		TCPAckReceivng, TCPDataReceiving
	};
	...
	void routerButtonClicked() {
		...
		if (pcap_findalldevs_ex(PCAP_SRC_IF_STRING, NULL, &alldevs, errbuf) != -1)
		{
			...
			pcap_compile(adhandle, &fcode, "icmp or udp or tcp", 1, 0xff);
	...
	void calcHeaderChecksum(ProtocolType p) {
		...
		case TCP:
			...
			sum1 += (iph->getSrcAddr() >> 16 & 0xffff) + (iph->getSrcAddr() & 0xffff);
			sum1 += 6;
			sum1 += iph->getTotalLen() - 20;
			for (int i = 0; i < 5; ++i)
				sum1 += (tcph->getWord(i) >> 16) + (tcph->getWord(i) & 0xffff);
			for (int i = 0; i < (iph->getTotalLen() - 39) / 2; ++i)
				sum1 += ((uint16_t)sentEthernetData[54 + 2 * i] << 8) + (uint16_t)sentEthernetData[54 + 2 * i + 1];
			sum2 = (uint16_t)((sum1 >> 16) + (sum1 & 0xffff));
			tcph->setChecksum(0xffff - sum2);
			break;
	...
	void generateSentAethernetData(ProtocolType p, bool isPreambleReversed) {
		...
		if (p == UDP) {
			...
			generateAethernetDataFromByte(0x00);
			generateAethernetDataFromByte(0x00);
			string token = cmd.getDestName(0);
			uint8_t tokenLen = token.length();
			generateAethernetDataFromByte(tokenLen - 7);
			for (int j = 7; j < tokenLen; ++j)
				generateAethernetDataFromByte(token.at(j));

			for (int i = 1; i < cmd.getLength(); ++i) {
				token = cmd.getDestName(i);
	...
	void generateSentEthernetData(ProtocolType p, uint64_t srcMac, uint64_t destMac) {
		...
		for (int i = 0; i < 2; ++i)
			for (int j = 0; j < 4; ++j)
				...
				case TCP:
					sentEthernetData[34 + 4 * i + j] = tcph->getByte(i, j);
