...
void MainComponent::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill) {
    ...
    for (auto channel = 0; channel < maxOutputChannels; ++channel) {
        if ((!activeInputChannels[channel] || !activeOutputChannels[channel]) || maxInputChannels == 0) {
            ...
            float freq1 = 1000, freq2 = 10000;
            float amp = 0.1;
            float dPhasePerSample1 = 2 * juce::MathConstants<double>::pi * (freq1 / (float)sampleRate);
            float dPhasePerSample2 = 2 * juce::MathConstants<double>::pi * (freq2 / (float)sampleRate);
            float initPhase = 0;
            ...
            case Playing:
                for (int i = 0; i < bufferSize; ++i) {
                    outBuffer[i] +=
                        amp * (sin(dPhasePerSample1 * i + initPhase) + sin(dPhasePerSample2 * i + initPhase));
                }
