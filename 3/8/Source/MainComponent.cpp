	...
	for (auto channel = 0; channel < maxOutputChannels; ++channel) {
		if ((!activeInputChannels[channel] || !activeOutputChannels[channel]) || maxInputChannels == 0) {
			...
			case AethernetReceiving:
				...
				while (begin && receivedData.size()) {
					...
					if (++bitsReceived == BYTE_NUM * 8 * BIT_WIDTH) {
						...
						calcICMPChecksum();
						generateSentEthernetData(NODE3_HOTSPOT_MAC, NODE1_AETHERNET_MAC);
						originalData.clear();
						receivedData.clear();
						changeState(EthernetNode1RequestSending);
						break;
					}
				}
			...
			case AethernetReplyReceiving:
				...
				while (begin && receivedData.size()) {
					...
					if (++bitsReceived == PACKET_LENGTH * 8 * BIT_WIDTH) {
						...
						icmph->setType(0);
						calcICMPChecksum();
						generateSentEthernetData(NODE1_AETHERNET_MAC, NODE3_HOTSPOT_MAC);
			...
			case EthernetNode1RequestSending:
			{
				...
				calcICMPChecksum();
				generateSentEthernetData(NODE1_AETHERNET_MAC, NODE3_HOTSPOT_MAC);
				changeState(EthernetNode1ReplySending);
				break;
			}
