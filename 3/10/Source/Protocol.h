...
public:
	auto setHardwareType(uint16_t type) { hardwareType = type; }

	auto setprotocolType(uint16_t type) { protocolType = type; }

	auto setHardwareSize(uint8_t size) { hardwareSize = size; }

	auto setProtocolSize(uint8_t size) { protocolSize = size; }

	auto setOpcode(uint16_t code) { opcode = code; }

	auto setSenderMac(uint64_t s) { senderMac = s; }

	auto setSenderIP(uint32_t ip) { senderIP = ip; }

	auto setTargetMac(uint64_t t) { targetMac = t; }

	auto setTargetIP(uint32_t ip) { targetIP = ip; }

	auto getHardwareType() { return hardwareType; }

	auto getprotocolType() { return protocolType; }

	auto getHardwareSize() { return hardwareSize; }

	auto getProtocolSize() { return protocolSize; }

	auto getOpcode() { return opcode; }

	auto getSenderMac() { return senderMac; }

	auto getSenderIP() { return senderIP; }

	auto getTargetMac() { return targetMac; }

	auto getTargetIP() { return targetIP; }

	uint32_t getWord(int index) {
		uint32_t word = 0;
		switch (index) {
		case 0:
			word += (uint32_t)hardwareType << 16;
			word += (uint32_t)protocolType;
			break;

		case 1:
			word += (uint32_t)hardwareSize << 24;
			word += (uint32_t)protocolSize << 16;
			word += (uint32_t)opcode;
			break;

		case 2:
			word += (uint32_t)(senderMac >> 16);
			break;

		case 3:
			word += (uint32_t)(senderMac & 0xffff) << 16;
			word += (uint32_t)(senderIP >> 16) & 0xffff;
			break;

		case 4:
			word += (uint32_t)(senderIP & 0xffff) << 16;
			word += (uint32_t)(targetMac >> 32) & 0xffff;
			break;

		case 5:
			word += (uint32_t)(targetMac & 0xffffffff);
			break;

		case 6:
			word += (uint32_t)targetIP;
			break;
