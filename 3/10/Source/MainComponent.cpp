		...
		if ((!activeInputChannels[channel] || !activeOutputChannels[channel]) || maxInputChannels == 0) {
			...
			case EthernetNode1ARPReceiving:
			{
				...
				if ((res = pcap_next_ex(adhandle, &pktHeader, &pktData)) >= 0) {
					if (res > 0 && *((uint8_t*)(pktData + 12)) == 8 && *((uint8_t*)(pktData + 13)) == 6) {
						sentData.clear();
						sentData.insert(sentData.end(), preambleWave1.begin(), preambleWave1.end());
						for (int i = 0; i < 4; ++i)
							generateSentAethernetDataFromByte(*((uint8_t*)(pktData + 38 + i)));

						changeState(AethernetNode1ARPSending);
						break;
					}
				}
			...
			case AethernetNode1ARPSending:
				...
				for (int i = 0; i < bufferSize; ++i, ++readPointer) {
					if (readPointer < sentData.size())
						...
						receivedData.clear();
						originalData.clear();
						changeState(AethernetNode1ARPReplyReceiving);
						break;
					}
				}
				break;
			...
			case AethernetNode2ARPReceiving:
				...
				while (begin && receivedData.size()) {
					...
					if (++bitsReceived == 4 * 8 * BIT_WIDTH) {
						...
						arph = new ARPHeader();
						arph->setHardwareType(1);
						arph->setprotocolType(0x0800);
						arph->setHardwareSize(6);
						arph->setProtocolSize(4);
						arph->setOpcode(1);
						arph->setSenderMac(NODE1_MAC_ADDR);
						arph->setSenderIP(NODE1_IP);
						arph->setTargetMac(0);
						arph->setTargetIP(NODE2_IP);
						generateSentEthernetData(ARP, NODE1_MAC_ADDR, 0xffffffffffff);

						originalData.clear();
						receivedData.clear();
						changeState(EthernetNode2ARPSending);
						break;
					}
				}
			...
			case EthernetNode2ARPSending:
				pcap_sendpacket(adhandle, sentEthernetData, ARP_PACKET_LENGTH);
				arph->setOpcode(2);
				arph->setSenderMac(NODE2_MAC_ADDR);
				arph->setSenderIP(NODE2_IP);
				arph->setTargetMac(NODE1_MAC_ADDR);
				arph->setTargetIP(NODE1_IP);
				generateSentEthernetData(ARP, NODE2_MAC_ADDR, NODE1_MAC_ADDR);
				changeState(EthernetNode2ARPReplySending);
				break;
			...
			case EthernetNode2ARPReplySending:
				...
				for (int i = 0; i < 6; ++i)
					generateSentAethernetDataFromByte((uint8_t)((NODE2_MAC_ADDR >> ((5 - i) * 8)) & 0xff));

				setReadPointer(0);
				changeState(AethernetNode2ARPReplySending);
				break;
			...
			case AethernetNode2ARPReplySending:
				...
				for (int i = 0; i < bufferSize; ++i, ++readPointer) {
					if (readPointer < sentData.size())
						...
						receivedData.clear();
						originalData.clear();
						changeState(AethernetNode2ICMPReceiving);
						break;
					}
				}
				break;
			...
			case AethernetNode1ARPReplyReceiving:
				...
				if (begin) {
					...
					originalData.clear();
					receivedData.clear();
					changeState(EthernetNode1ARPReplySending);
					begin = false;
				}
				break;
			...
			case EthernetNode1ARPReplySending:
				pcap_sendpacket(adhandle, sentEthernetData, ARP_PACKET_LENGTH);
				changeState(EthernetNode1ICMPReceiving);
				break;
				...
				if ((res = pcap_next_ex(adhandle, &pktHeader, &pktData)) >= 0) {
					if (res > 0 && *((uint8_t*)(pktData + 34)) == 8) {
						...
						if (iph->getDestAddr() == NODE2_IP) {
							...
							setReadPointer(0);
							changeState(AethernetNode1ICMPSending);
						}
			...
			case AethernetNode1ICMPSending:
				...
				for (int i = 0; i < bufferSize; ++i, ++readPointer) {
					if (readPointer < sentData.size())
						...
						receivedData.clear();
						originalData.clear();
						changeState(AethernetNode1ICMPReplyReceiving);
						break;
					}
				}
				break;
			...
			case AethernetNode2ICMPReceiving:
				...
				while (begin && receivedData.size()) {
					...
					if (++bitsReceived == BYTE_NUM * 8 * BIT_WIDTH) {
						...
						calcICMPChecksum();
						generateSentEthernetData(ICMP, NODE1_MAC_ADDR, NODE2_MAC_ADDR);
						originalData.clear();
						receivedData.clear();
						changeState(EthernetNode2ICMPSending);
						break;
					}
				}
			...
			case EthernetNode2ICMPSending:
				pcap_sendpacket(adhandle, sentEthernetData, PACKET_LENGTH);
				iph->setDestAddr(NODE1_IP);
				iph->setSrcAddr(NODE2_IP);
				iph->setHeaderChecksum();
				...
				icmph->setType(0);
				calcICMPChecksum();
				generateSentEthernetData(ICMP, NODE2_MAC_ADDR, NODE1_MAC_ADDR);
				changeState(EthernetNode2ICMPReplySending);
				break;

			case EthernetNode2ICMPReplySending:
				pcap_sendpacket(adhandle, sentEthernetData, PACKET_LENGTH);
				generateSentAethernetData(true);
				changeState(AethernetNode2ICMPReplySending);
				break;
			...
			case AethernetNode1ICMPReplyReceiving:
				...
				while (begin && receivedData.size()) {
					...
					if (++bitsReceived == PACKET_LENGTH * 8 * BIT_WIDTH) {
						...
						delete iph;
						delete icmph;
						changeState(EthernetNode1ICMPReplySending);
						break;
					}
				}
